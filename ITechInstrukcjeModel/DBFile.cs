...
namespace ITechInstrukcjeModel
{
    ...
    public class DBFile
    {
        ...
        private void Save(ITechInstrukcjeModel.FileData data)
        {
            using (var conn = OpenConnection())
        ...
        public Stream Get(Guid DocId)
        {
            ...
            var cmd = new SqlCommand(
                "SELECT Content.PathName() [Path], GET_FILESTREAM_TRANSACTION_CONTEXT() [TransactionContext] " +
                "FROM dbo.FileContent F WHERE F.FileId=@DocId", conn, tran);
        ...
        public Stream Get(int DocId)
        {
            ...
            var cmd = new SqlCommand(
                "SELECT Content.PathName() [Path], GET_FILESTREAM_TRANSACTION_CONTEXT() [TransactionContext] " +
                "FROM dbo.FileContent F WHERE F.Dokument_Id=@DocId", conn, tran);
            cmd.Parameters.AddWithValue("@DocId", DocId);
